...
SIGNATUR(CONT_STORE);
SIGNATUR(TEST1);
INCLUDE_LIBRARY(test_lib)
INCLUDE_LIBRARY(hello)
struct X{
   int x;
   int y; 
   char str[100];
...
START(4)
    PUSH_LITERAL(LAMBDA(test_lib,0))
    APPLICATE(0,1345)
    PUSH_LITERAL(LAMBDA(test_lib,0))
    APPLICATE(0,45)
    printf("start %d\n", *(GLOBAL_BOUND(test_lib,0).data.boolean_val));
    PUSH_LITERAL(LAMBDA_BASE(current_output_port))
    APPLICATE(0,342)
    PUSH_LITERAL(LAMBDA(newline,-1))
    APPLICATE(1,34)
    PUSH_LITERAL(scheme_new_boolean(1))
    PUSH(GLOBAL_BOUND(test_lib, 1))
    APPLICATE(1,33)
